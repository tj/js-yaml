...
tokenize: function (str) {
  var token, captures, ignore, input,
      ...
      stack = []
  while (str.length) {
    for (var i = 0, len = YAML.tokens.length; i < len; ++i)
      if (captures = YAML.tokens[i][1].exec(str)) {
        token = [YAML.tokens[i][0], captures],
        str = str.replace(YAML.tokens[i][1], '')
        switch (token[0]) {
          case 'comment':
            ignore = true
            break
...
exports.tokenize = YAML.tokenize
exports.eval = YAML.eval
